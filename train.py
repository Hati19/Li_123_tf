...
img_cols=384
num_epoch=100
input_training_dir='/users/PCCF0019/ccf0071/Downloads/OAI/train_all/'
#input_training_dir='/users/PCCF0019/ccf0071/Downloads/OAI/train_small/'
...
#print(train_id)
#print(test_id)
#with open('output/test_id.txt', 'w') as filehandle:  
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        if iter % 2 == 0:
            current_batch,current_label=get_data(train_id[:2])
            test_example=current_batch[..., np.newaxis]
            test_example_gt=current_label[..., np.newaxis]
            #sess_results = sess.run([cost,gen_optim],feed_dict={X:current_batch,Y:current_label})
            #test_example =   train_images[:2,:,:,:]
            #test_example_gt = train_labels[:2,:,:,:]
            sess_results=sess.run(y_pred, feed_dict={X: test_example})
            ...
            plt.figure()
            plt.imshow(np.squeeze(test_example, axis=( 2)),cmap='gray')
            plt.axis('off')
            plt.title('Original Image')
            plt.savefig('train_change/'+str(iter)+"a_Original_Image.png")

            plt.figure()
            plt.imshow(np.squeeze(test_example_gt, axis=( 2)),cmap='gray')
            plt.axis('off')
            plt.title('Ground Truth Mask')
            plt.savefig('train_change/'+str(iter)+"b_Original_Mask.png")

            plt.figure()
            plt.imshow(np.squeeze(sess_results, axis=( 2)),cmap='gray')
            plt.axis('off')
            plt.title('Generated Mask')
            plt.savefig('train_change/'+str(iter)+"c_Generated_Mask.png")
            
            plt.figure()
            plt.imshow(threshold_by_otsu(np.squeeze(sess_results, axis=( 2))),cmap='gray')
            plt.axis('off')
            plt.title('Generated Mask_threshold_by_otsu')
            plt.savefig('train_change/'+str(iter)+"d_Generated_ostu_Mask.png")
...
def eval1(imgs_val,imgs_mask_val, iter_time, best_auc_sum, phase='train'):
        total_time, auc_sum = 0., 0.
        #print('In Eval--------')
        if np.mod(iter_time, FLAGS.eval_freq) == 0:
            num_data, imgs, vessels, masks = None, None, None, None
            num_data=imgs_val.shape[0]
            imgs=imgs_val
            vessels=imgs_mask_val
            #print(num_data)
            '''if phase == 'train':
            ...
            for iter_ in range(num_data):
                ...
                generated_vessel = model.sample_imgs(x_img)
                total_time += (time.time() - start_time)
                y_img_pred=np.squeeze(generated_vessel, axis=(0, 3))
                dice_loss.append(dice(y_img_pred,y_img))
                print('dice--%.3f\n'%dice(y_img_pred,y_img))
                otsu_dice_loss.append(dice(threshold_by_otsu(y_img_pred),y_img))
                print('otsudice--%.3f\n'%dice(threshold_by_otsu(y_img_pred),y_img))
                plt.figure()
    	        plt.imshow(np.squeeze(x_img, axis=( 2)),cmap='gray')
    	        plt.axis('off')
    	        plt.title('Original Image')
    	        plt.savefig('train_change/'+str(iter)+"a_Original_Image.png")

	            plt.figure()
	            plt.imshow(np.squeeze(y_img, axis=( 2)),cmap='gray')
        	    plt.axis('off')
        	    plt.title('Ground Truth Mask')
        	    plt.savefig('train_change/'+str(iter)+"b_Original_Mask.png")
        
        	    plt.figure()
        	    plt.imshow(np.squeeze(generated_vessel, axis=( 2)),cmap='gray')
        	    plt.axis('off')
        	    plt.title('Generated Mask')
        	    plt.savefig('train_change/'+str(iter)+"c_Generated_Mask.png")
        	    
        	    plt.figure()
        	    plt.imshow(threshold_by_otsu(np.squeeze(generated_vessel, axis=( 2))),cmap='gray')
        	    plt.axis('off')
        	    plt.title('Generated Mask_threshold_by_otsu')
        	    plt.savefig('train_change/'+str(iter)+"d_Generated_ostu_Mask.png")
	    print('\n-------- validation Loss---------------')
